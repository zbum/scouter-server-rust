...
pub fn set_configure_server(
    ...
    table.insert("message".into(), Value::Text("Runtime config change not supported yet".into()));
...
pub fn list_configure_server(
    ...
    table.insert("net_udp_listen_port".into(), Value::Decimal(config.net_udp_listen_port as i64));
    table.insert("net_tcp_listen_port".into(), Value::Decimal(config.net_tcp_listen_port as i64));
    table.insert("net_http_port".into(), Value::Decimal(config.net_http_port as i64));
    table.insert("db_dir".into(), Value::Text(config.db_dir.clone()));
    table.insert("db_keep_days".into(), Value::Decimal(config.db_keep_days as i64));
    table.insert("xlog_queue_size".into(), Value::Decimal(config.xlog_queue_size as i64));
    table.insert("counter_queue_size".into(), Value::Decimal(config.counter_queue_size as i64));
